...
open class AlertView: UIView {
    ...
    public func configureAlert<viewItem>(item: viewItem, acceptHandler: (() -> ())? = nil) {
        ...
        baseView.configureShadowWithCorner(shadowColor: .darkGray, shadowOpacity: 0.2)
        acceptButton.backgroundColor = .darkGray
        acceptButton.setTitleColor(.lightGray, for: .normal)
        acceptButton.cornerRadius = 4
        
        cancelButton.borderColor = .darkGray
        ...
        if let data = item as? AlertViewItem {
            ...
            if let messageText = data.messageText {
                messageLabel.textColor = .darkGray.withAlphaComponent(0.8)
